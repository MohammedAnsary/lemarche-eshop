<?php
	class Product {
		...
		function viewProducts() {
			$messages = array();
			$viewProduct = json_decode(DB::query('SELECT name,price,avatar FROM product', 1), true);
			if($viewProduct['status'] = 'OK')
			{
				return DB::query('SELECT name,price,avatar FROM product', 1);
			}
				$messages['products_list'] = 'There are no products in the DataBase';
